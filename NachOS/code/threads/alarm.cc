...
Alarm::CallBack() 
{
    ...
    {	
        if (!interrupt->AnyFutureInterrupts()) // turn off the timer
        {
            timer->Disable();	
        }
    }
    // there's someone to preempt
    else if (kernel->scheduler->getSchedulerType() == RR || kernel->scheduler->getSchedulerType() == Priority ) 
    {
        cout << "=== interrpt->YieldOnReturn ===" << endl;
